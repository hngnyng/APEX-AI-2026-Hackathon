...
export const AnalyticsEngine = {
  ...
  analyzeProfile(studentData) {
    ...
    // Positives: sleep hours, recreation time. Negatives: excessive work hours (>50 hrs/wk), high stress type
    ...
    let examReadiness = 0;
    if (exams.length === 0) {
      examReadiness = 100; // No exams is technically fully prepared
    } else {
      examReadiness = Math.round((avgExamPrep / 5) * 100);
      // Deduct for urgent critical exams
    ...
    academicStress = Math.max(10, Math.min(95, academicStress)); // cap at 95% to avoid absolute panic
  ...
  generateAdvice(archetypeId, weakSubjects, exams, sleepHours, workload) {
    ...
    // Rule 2: High Study Volume, Low GPA (Struggler)
    ...
    // Rule 5: Focus on Weakest Subject
    ...
    // General fallback advice if list is short
    if (list.length < 3) {
      list.push({
        id: "adv-standard-1",
  ...
  generateStudyPlan(archetypeId, weakSubjects, avgGPA) {
    ...
    // Customize milestones based on archetype
    ...
    // Append a task for the weakest subject
    if (weakSubjects.length > 0) {
